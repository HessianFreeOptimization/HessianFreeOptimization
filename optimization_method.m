function [llrecord, errrecord, paramsp, eval_fs, eval_gs, step_size] = optimization_method(algorithm, eta, maxIter, fid, params, paramsinit)
...
function grad = calcu_grad(paramsp)
    ...
    % NOTE: grad is the gradient w.r.t. loss other than Log-likelihood.
    grad = -grad;
end
...
for epoch = 1:maxIter
    ...
    if strcmp(algorithm, 'gradient descent')
        ...
        j = 0;
        oldll = ll;
        [ll, err] = computeLL(paramsp + step*v, indata, outdata);
        while j < 60
            if -ll <= -oldll - 0.01*step*v'*v
                break;
            else
                step = 0.8*step;
                j = j + 1;
